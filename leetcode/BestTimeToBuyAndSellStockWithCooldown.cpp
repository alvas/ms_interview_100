...
public:
    int maxProfit(vector<int>& prices) {
        ...
        for (int i = 1; i < sz; ++i) {
            ...
            for (int j = i; j > 0; --j) {
                l[i] = max((j >= 3 ? g[j - 3]: 0) + max(diff, 0), (j >= 4 ? g[j - 4]: 0) + l[i - 1] + diff);
                g[i] = max(l[i], g[i]);
                g[i] = max(g[i], g[i - 1]);
            }
        }
    ...
    int maxProfit_slow(vector<int>& prices) {
        int profit = 0;
        getProfit(prices, 0, 0, profit);
        return profit; 
    }

    void getProfit(vector<int>& prices, int b, int tmp, int &profit) {
        int sz = prices.size();
        ...
        for (int i = b; i < sz; ++i) {
            for (int j = i + 1; j < sz; ++j) {
                ...
                if (diff > 0) {
                    // j + 2 cooldown
                    getProfit(prices, j + 2, tmp + diff, profit);
                }
            }
        }

        // don't forget to update profit after loop through all price, otherwise, profit would not be updated from tmp.
        profit = max(tmp, profit);
    }
};
...
int main() {
    ...
    std::cout << sln.maxProfit_slow(prices) << std::endl;
